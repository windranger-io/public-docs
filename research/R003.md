...
Token burn mechanisms to burn supply. Think of a stablecoin that wants to reduce its supply, or a staking contract in which someone want to remove the initial funds and burn the LP token.

**Alternative** <br/>
Contracts that don't have this function can:

- Send tokens to the [0x0000... address](https://etherscan.io/address/0x0000000000000000000000000000000000000000). This is a coomonly accepted method for burning that is also token agnostic.

  - Recognized by etherscan as the address often associated with token burn events ([screenshot](https://share.getcloudapp.com/BluWZknR))

  - Recognized and accounted for by various exchanges (ie CoinGecko) when calculating circulating supply

- Apart from the 0x000 address, the [0x0...dead wallet](https://etherscan.io/address/0x000000000000000000000000000000000000dead) appears to be the most common, widely accepted for the burning of tokens. Articles/anecdotes referencing this address:

  - Also recognized by Etherscan as a common burn address ([screenshot](https://share.getcloudapp.com/2Nu7X4eJ))

  - Cited as the token burning address in [Mastering Ethereum](https://github.com/ethereumbook/ethereumbook/blob/develop/06transactions.asciidoc#special-transaction-contract-creation)
